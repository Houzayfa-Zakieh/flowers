    ...
    return (
        <>
            <div ref={dropdownRef} className="relative inline-block z-1 text-left ">
                <button type="button" onClick={() => setOpen(!open)} className="h-[45px] text-[#666] cursor-pointer !font-thin inline-flex items-center justify-between w-50 px-4 py-2 bg-[#fff] border border-[#dfdfdf] text-[14px] outline-none">
                    {selectedOption}
                ...
                {open && (
                    <div className="absolute right-0 w-[100%] mt-[2px] origin-top-right bg-[#fff] border border-[#dfdfdf] rounded-md shadow-lg">
                        <div className="py-1">
                            {options.map((option) => (
